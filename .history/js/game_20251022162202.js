...
function resetGame() {
  ...
  // 1. Réinitialiser les variables de jeu
  count = 0;
  firstGuess = "";
  secondGuess = "";
  previousTarget = null;

  // 2. Vider la grille (supprimer toutes les cartes)
  grid.innerHTML = "";

  // 3. Mélanger à nouveau les cartes
  gameGrid = cardsArray.concat(cardsArray);
  gameGrid.sort(() => 0.5 - Math.random());

  //4. Recréer les cartes
...
let firstGuess = ""; // Ajouter des variables pour suivre les premières et deuxièmes sélections
let secondGuess = "";
let previousTarget = null; // Garder une trace de la carte précédemment cliquée
let delay = 1200; // Délai en millisecondes avant de réinitialiser les suppositions
...
// Elle génère un nombre aléatoire entre -0.5 et 0.5, ce qui entraîne un ordre aléatoire des éléments du tableau.

// Créer les cartes avec le tableau dupliqué
gameGrid.forEach((item) => {
  ...
  const back = document.createElement("div");
  back.classList.add("back");
  back.style.backgroundImage = `url(${item.img})`; //on va appeler l'image à partir du tableau cardsArray

  // Ajouter la carte à la grille, ainsi que le recto et le verso de chaque carte
  card.appendChild(front); // front va dans card
  card.appendChild(back); // back va dans card
  grid.appendChild(card); // card va dans grid
});
...
// Fonction resetGuesses
const resetGuesses = () => {
  // Réinitialiser pour la prochaine paire
  firstGuess = "";
  secondGuess = "";
  count = 0;
  const selected = document.querySelectorAll(".selected"); // Retirer la classe 'selected' des cartes non matchées
  selected.forEach((card) => {
    card.classList.remove("selected");
  });
};

// Écouter les clics sur la grille
grid.addEventListener("click", function (event) {
  ...
  // Ne pas permettre de cliquer sur une carte déjà matchée et sélectionnée
  if (
  ...
  if (count < 2) {
    ...
    if (count === 1) {
      ...
      clicked.parentNode.classList.add("selected"); // on ajoute "selected" sur le parent
    } else {
